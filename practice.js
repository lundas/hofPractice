...
var getUserTweets = function(tweets, user) {
  var userTweets = [];
  _.each(tweets, function (obj, index, tweets) {
    if (obj['user'] === user) {
      userTweets.push(obj);
    }
  });
...
var applyCoupon = function (groceries, coupon) {
  return _.map(groceries, function (grocery, index, groceries) {
    grocery.salePrice = Math.round(grocery.price.slice(1) * (1 - coupon) * 100) / 100;
    grocery.salePrice = '$' + grocery.salePrice.toString();
    return grocery;
  });
};
